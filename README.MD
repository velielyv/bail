    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const m of messages) {
            ...
            await sock.sendMessage(m.key.remoteJid!, { 
                text: 'Halo! Saya bot WhatsApp menggunakan baileys 🤖' 
            })
        }
    })
...
WhatsApp menyediakan API multi-device yang memungkinkan baileys terautentikasi sebagai klien WhatsApp sekunder melalui QR code atau pairing code.
...
// Pairing Kustom
if (!sock.authState.creds.registered) {
    const pair = "BAIL4444" // 8 karakter
    const number = '628xxxx'
...
2. Secara default, baileys menggunakan konfigurasi Chrome. Untuk koneksi seperti desktop (untuk riwayat pesan lebih banyak), gunakan:
...
baileys tidak menyertakan penyimpanan default untuk obrolan, kontak, atau pesan. Namun, implementasi in-memory sederhana disediakan. Store mendengarkan update obrolan, pesan baru, dll., untuk menjaga data tetap terkini.
...
2. baileys punya fungsi untuk pratinjau tautan.
...
await sock.sendMessage(
    jid,
    {
        text: 'Halo, ini dikirim menggunakan https://npmjs.com/baileys'
    }
)
```
...
baileys tidak muat seluruh buffer ke memori; enkripsi sebagai stream.
...
baileys dirancang untuk fungsi kustom. Tambahkan ekstensi sendiri tanpa fork.

### Aktifkan Level Debug di Log baileys
